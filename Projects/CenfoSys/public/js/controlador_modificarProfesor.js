...
function obtenerDatosActualizar() {
  ...
  if (!bError) {
    ...
    let sFotografia = profesor[31];
    if (getTempFotografiaPeril() == 'audaq6qqr5v6ispj4oua') {
      sFotografia = profesor[31];
    } else {
      sFotografia = getTempFotografiaPeril();
    }
...
function getHorario() {
  let listaHorario = buscarProfesorPorId(getProfesor()[0])[24];
...
function getIdiomas() {
  let listaIdiomas = buscarProfesorPorId(getProfesor()[0])[28];
